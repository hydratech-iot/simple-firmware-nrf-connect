...
float madgwick_get_roll_radians()
{
    if (!anglesComputed)
    {
        computeAngles();
...
float madgwick_get_pitch_radians()
{
    if (!anglesComputed)
    {
        computeAngles();
...
float madgwick_get_yaw_radians()
{
    if (!anglesComputed)
    {
        computeAngles();
...
static void computeAngles()
{
	roll           = atan2f(2.0f * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2));
	pitch          = asinf(2.0f * (q0 * q2 - q1 * q3));
	yaw            = atan2f(2.0f * (q1 * q2 + q0 * q3), 1 - 2 * (q2 * q2 + q3 * q3));
	anglesComputed = 1;
}
